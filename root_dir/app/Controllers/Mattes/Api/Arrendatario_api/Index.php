...
class Index extends ResourceController
{
    ...
    public function index(){
      ...
      (SELECT pickture FROM propetyfiles WHERE property.id = propetyfiles.id_propety AND pickture != " " AND propetyfiles.deleted_at  = "0000-00-00 00:00:0" AND pickture not like "%.mp4%" LIMIT 1) AS imagen, 
      ...
      (SELECT propertyrating.property_count FROM propertyrating WHERE property.id = propertyrating.id_property limit 1) AS estrellas
       FROM property where property.verified = 1 and property.rent = 0 AND property.deleted_at = "0000-00-00 00:00:00"';
      ...
      // $data = $model->getBusqueda($query_result);
        ...
        $ordenamiento = ' order by positioning  desc ';
        ...
        .$condicion_limpieza.$condicion_seguridad.$condicion_lavadora.$condicion_cocina.$ordenamiento;
        $data = $model->getBusqueda($query_result);
       return $this->respond($data, 200);   
