...
let datacitas = $('#citas_agentes').DataTable({
    'ajax': {
        'url': `${BASE_URL}Mattes/Api/Arrendatario_api/Agendarcita_rest/get_citas_agente`,
        'data': {
            'id_crofter' : id_user
        },
        'type': 'post',
    ...
    columns: [
        ...
        },
        {
            ...
            render: function ( data, type, row ) {
                return `${row.agente} ${row.ap_agente} ${row.am_agente}`;
            }
        },
        {
            data: 'universidad'
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                if(row.status == 501){
                    return `<button type="button" id="${row.id}" data-index="${row.id_renter}" data-crofter="${row.id_crofter}" data-toggle="modal" data-target="#show_reasignar_cita" class="btn btn-warning btn-res"> <i class="fa fa-clock-o" aria-hidden="true"></i> Reasignar</button>`
                } else if(row.status == 502){
                    return ` `
                }
                else {
                    return `<div class="d-flex"><button type="button" id="${row.id}" data-index="${row.id_renter}" data-crofter="${row.id_crofter}" data-property="${row.id_property}" data-toggle="modal" data-target="#show_aceptar_cita" class="btn btn-aceptar btn-can"><i class="fa fa-check-circle" aria-hidden="true"></i> Aceptar</button>
                     <button type="button" id="${row.id}" data-index="${row.id_renter}" data-crofter="${row.id_crofter}" data-toggle="modal" data-target="#show_cancelar_cita" class="btn btn-danger btn-can"><i class="fa fa-ban" aria-hidden="true"></i> Cancelar</button></div>`
                }
...
let datapreguntas = $('#preguntas_agentes').DataTable({
    'ajax': {
        'url': `${BASE_URL}Mattes/Api/Arrendador_api/Conversacion_rest/get_questions_agente`,
        'data': {
        },
        'type': 'post',
    },
    columns: [
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                if(row.status == 0){
                    return `<button type="button" id="${data}" data-question="${row.question}" data-renter="${row.user_id}" data-toggle="modal" data-target="#show_responder" class="btn btn-warning btn-question"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Responder</button>`
                } else{
                    return ``
                }
            }
...
let datamensajes = $('#mensajes_agentes').DataTable({
    'ajax': {
        'url': `${BASE_URL}Mattes/Api/Arrendador_api/Conversacion_rest/get_convers_agente`,
        'data': {
        },
        'type': 'post',
    },
    columns: [
        {
            ...
            render: function(data, type, row, meta) {
                ...
                return `<a href="${BASE_URL}conversacion-agente/${id_conversacion}" target="_blank">${data}</a>`
        ...
        },
        {
            ...
            render: function(data, type, row, meta) {
                //nombrencode = data.replace(/ /g, "-")
                return 'NO LEÍDO'
                //`<a href="${BASE_URL}Mattes/Arrendatario/Propiedad_detalle/index/${nombrencode}" target="_blank">${data}</a>`
