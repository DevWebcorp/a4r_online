$('#form_bancarios_inmobiliaria').submit(function() {
    ...
    const url = `${BASE_URL}Mattes/Api/Arrendador_api/Datos_empresa/datos_banco`;
...
function get_bancarios() {
    const url = `${BASE_URL}Mattes/Api/Arrendador_api/Datos_empresa/get_bancario`;
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res.length > 0) {
                ...
                $('#nombre_inmobi').val(res[0].full_name);
                $('#banco_nombre').val(res[0].bank_name);
                $('#clabe_banco').val(res[0].interbank_number);
            }
        }
    });
