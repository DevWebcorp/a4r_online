...
class Registro_estudiante extends BaseController
{
	public function index()
	{
		...
		if( $session->get('logged_in') != null){
			...
			$data_header['title'] = "Registro estudiante";
			$data_header['description'] = "Registro de los datos del estudiante";
			...
			echo view('Login/Registro_estudiante' ,  $data_header);
	...
	public function register(){
		helper('sendmail');
		$url_confirmation = base_url()."/Registro_estudiante/confirm_email/";
		//$username=$_POST['name'];
		...
		switch(true){
			...
			case( $this->user_exist($email) ):
				$error = "El correo ya se encuentra registrado. Si ha olvidado su contraseña, puede recuperala en el siguiente enlace <a href=\"".base_url()."/register/password_recover\">Recuperar contraseña</a>";
				break;
			default:
				break;
		}
		...
		if(isset($error)){
			...
			if($id > 0){
				...
				$data_header['success'] = "Tu usuario se ha creado con éxito. Se ha enviado un correo a ".$email.". Por favor confirma tu correo para concluir el proceso.";
				$subject = "Gracias por unirte a Mattes";
				$activation_token = str_replace( '/' , '&&&' , $activation_token);
				//$data['user'] = $username;
				$data['url'] = $url_confirmation.urlencode($activation_token);
				...
				echo view('Mattes/Menu_principal');
				echo view('Login/Registro_estudiante' ,  $data);
				echo view('Mattes/Footer');
			}
		}
	}
