<?php namespace App\Models;
	...
	class User extends Model {
		...
		public function getToken($email){
			$query = sprintf('SELECT email , activation_token FROM users WHERE email= %s' , $this->db->escape( $email ) );
			$result = $this->db->query( $query );
			return $result->getResult();
		}

		public function getIdFromMail($email){
			$query = sprintf('SELECT id FROM users WHERE email= %s' , $this->db->escape( $email ));
			$result = $this->db->query( $query );
			return $result->getResult();
		}


		public function getIdIdentify($id){
			$data=$this->db->query("SELECT * FROM hcv_identity WHERE id_user='".$id."'");

			return $data->getResult();
		}

		public function getPathIdentify($id){
			$data=$this->db->query("SELECT path FROM hcv_identity WHERE id_user='".$id."'");
