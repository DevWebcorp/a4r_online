...
$("#universidad").keyup(function() {
    var search = document.getElementById("universidad").value;
    console.log('Busqueda: ' + search);
    var url_str = `${BASE_URL}Mattes/Api/Arrendador_api/Propiedad_ubicacion`;
    var universidads = {
        "search": search,
        "limit": "10"
    };
    ...
    $.ajax({
        ...
        type: 'POST',
        dataType: 'json',
        data: JSON.stringify(universidads),
        success: function(response) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                $('#univ').val(response[i].id_university);
                console.log("searchResult")
                $("#searchResult").append("<li value='" + id + "'  data-latitud ='" + response[i]['latitude'] + "'    data-longitud ='" + response[i]['longitude'] + "' >" + name + "</li>");
            }
            ...
            $("#searchResult li").bind("click", function() {
                ...
                $("#universidad").val(value);
                $("#univ").val(id_universidad);
                $("#latitud").val(latitud);
...
$(document).on('submit', '#form-busqueda', function() {
    ...
    /*  if (min_price > precio_minimo && min_price < max_price && max_price <= precio_maximo) {
            alert("El rango debe de ser entre $" + precio_minimo + " y $" + precio_maximo);
        }
     */
    ...
    /*formData.append('precio_min', arr[0]);
    formData.append('precio_max', arr[1]);*/
        ...
        $(data).each(function(i, v) {
            ...
            if (radio) {
                ...
                let info =
                    `<table>
                       <tr>
                           <td><img id="img-1" class="img-fluid" style="width:200px; height: 150px;" src="${path}/${v.imagen}" alt="First slide"></td>
                            <td>
                                <p class="info-casa text-uppercase" style="color: #0088F7; margin-left:6px;">${v.name}</p>
                                ...
                                <a href="${BASE_URL}datos-propiedad/${nombrencode}" target="_blank">
                                <button class="btn-mattes px-4 py-1"> Detalles</button></a>
                            </td>
                            <td>
                            </td>
                       </tr>
                ...
                var sello = v.stamp_mattes == "1" ? `<img id = "${v.stamp_mattes}" class = "sello-mattes" src = "${BASE_URL}/mattes/assets/img/Iconos/Certificacion.png">` : `<div id = "${v.stamp_mattes}" class = "sello-mattes"></div>`;
                ...
                var posiciona = v.positioning == "1" ? `<img class = "posiciona-propiedad" src = "${BASE_URL}/mattes/assets/img/Iconos_Mattes/Iconos/Mattes_Posiciona tu popiedad.png">` : `<div class = "posiciona-propiedad"></div>`;

                let mayusname = v.name.toUpperCase();

                // console.log(nombrencode);

                let html = `<a  href="${BASE_URL}datos-propiedad/${nombrencode}" target="_blank">
                                <div class="grid-item" data-img="${v.imagen}">
                                      <figure>
                                         <img id="img-1" class=" w-100" src="${path}/${v.imagen}" alt="First slide">
                                     </figure>
                                     <p id="${v.name}" class="info-casa mt-2 text-center" style="margin-left: 0px !important;">${mayusname}</p>
