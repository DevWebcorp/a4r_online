$(document).on('submit', '#form_notificaciones', function() {
    ...
    const url = `${BASE_URL}Mattes/Api/Arrendador_api/Datos_propietario/datos_notificaciones`;
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.status == 200) {
                ...
                /* $('#notificaciones').attr('disabled', true); */
                location.href = BASE_URL + "home-alumno";
                //setTimeout(index_r(),3000);
...
function get_notificaciones() {
    const url = `${BASE_URL}Mattes/Api/Arrendador_api/Datos_propietario/get_notificaciones`;
    $.ajax({
        url: url,
        type: 'GET',
        dataType: 'json',
